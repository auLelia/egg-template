...
class UploadController extends Controller {
  ...
  async create() {
    ...
    let url = `/uploads/${filename.toString()}${extname}`;
    // 组装参数 stream
    const target = path.join(this.config.baseDir, 'app/public/uploads', `/${filename.toString()}${extname}`);
  ...
  async url() {
    ...
    const options = {
      url,
      dest: path.join(this.config.baseDir, 'app/public/uploads', `${filename}.${extname}`)
    };
    const newPath = '/public/uploads/' + filename + '.' + extname;
    try {
